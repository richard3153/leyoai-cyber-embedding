...
# leyoai-cyber-embedding

## 中文介绍

LeyoAI Cyber Embedding 模型为网络安全文本提供向量表示，适用于语义搜索、相似度计算和聚类分析。
...
model = AutoModelForCausalLM.from_pretrained(
    "FFZwai/leyoai-cyber-embedding",
    device_map="auto"
)
tokenizer = AutoTokenizer.from_pretrained("FFZwai/leyoai-cyber-embedding")

# Inference
prompt = "将以下安全事件描述转换为向量：..."
...
LeyoAI Cyber Embedding model provides vector representations for cybersecurity texts, suitable for semantic search, similarity computation, and clustering analysis.
...
# Inference
prompt = "Convert the following security incident description to a vector: ..."
inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
outputs = model.generate(**inputs, max_length=100)
response = tokenizer.decode(outputs[0], skip_special_tokens=True)
print(response)
